...
object GlobalUtil {
    ...
    val appPackage: String
        get() = VideoCollectionsApplication.context.packageName
     ...
     */
    val appName: String
        get() = VideoCollectionsApplication.context.resources.getString(VideoCollectionsApplication.context.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = VideoCollectionsApplication.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            VideoCollectionsApplication.context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            VideoCollectionsApplication.context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(VideoCollectionsApplication.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
    ...
    fun getString(resId: Int): String {
        return VideoCollectionsApplication.context.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return VideoCollectionsApplication.context.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return VideoCollectionsApplication.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = VideoCollectionsApplication.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            VideoCollectionsApplication.context.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = VideoCollectionsApplication.context.packageManager
